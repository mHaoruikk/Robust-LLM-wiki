...
```
<wiki-root>/
├── CLAUDE.md          ← Schema: scope, conventions, gaps, high-level direction
...
`CLAUDE.md` is the **schema file** — the single most important configuration. It tells the LLM the wiki's scope, naming conventions, open questions, and research gaps at a high level. The concrete page index lives in `wiki/index.md` (generated by `build-index`); `CLAUDE.md` does not duplicate it. Read `references/schema-guide.md` for what to put in it. Read it at the start of every session.
...
## Reminders
**⚠️ You MUST NOT do any read/write outside the Given Obsidian Vault! If you feel really necessary, stop and ask for user permission.**
